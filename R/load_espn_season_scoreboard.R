...
#'
#' @examples
#' try(load_espn_season_scoreboard(2020))
load_espn_season_scoreboard <- function(season){
  options(warn = -1)
  ...
  scoreboard <- try(load_espn_scoreboard(start_date),silent = TRUE)

  for(i in seq(start_date + 1,end_date,1)){
    date = as.Date(i,origin = "1970-01-01")

    temp <- try(load_espn_scoreboard(date),silent = TRUE)
